...
class Converter {
    ...
    @TypeConverter
    fun favoritesFromGson(value: String?): List<FavoriteTitle>? {
        val type = object : TypeToken<List<FavoriteTitle>>() {
        }.type
        return if (value == null) ArrayList() else gson.fromJson(value, type)
    }

    @TypeConverter
    fun favoritesToGson(value: List<FavoriteTitle>?): String {
    ...
    @TypeConverter
    fun typeToInt(value: TitleType): Int {
        return value.type
    }

    @TypeConverter
    fun intToType(value: Int): TitleType {
        return TitleType.from(value)
    }
