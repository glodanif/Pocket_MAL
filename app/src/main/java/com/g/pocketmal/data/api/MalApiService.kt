...
class MalApiService(sessionManager: SessionManager, private val oAuthConfig: OAuthConfig) : ApiService {
    ...
    override suspend fun getAccessToken(code: String, codeVerifier: String): Response<TokenResponse> {
        val url = MalApi.AUTH_BASE_URL + "token"
        return apiService.getAccessToken(url, oAuthConfig.clientId, "authorization_code", code, oAuthConfig.redirectUrl, codeVerifier)
    }

    override suspend fun getFirstListPage(type: TitleType, includeNsfw: Boolean): Response<ListResponse> {
        val typePath = if (type == ANIME) "animelist" else "mangalist"
        val nsfw = if (includeNsfw) "1" else "0"
        val url = "${MalApi.API_BASE_URL}users/@me/$typePath?fields=my_list_status{start_date,finish_date,num_times_rewatched,num_times_reread,tags}," +
    ...
    override suspend fun getUserInfo(userId: Int): Response<UserResponse> = apiService.getUserInfo(userId.toString())
    ...
    override suspend fun updateTitle(id: Int, params: HashMap<String, Any>, type: TitleType): Response<ListStatus> {
    ...
    override suspend fun getRankingList(titleType: TitleType, rankingType: RankingType, includeNsfw: Boolean, limit: Int, offset: Int): Response<RankingResponse> {
    ...
    override suspend fun search(query: String, type: TitleType, includeNsfw: Boolean): Response<SearchResponse> {
        val typePath = if (type == ANIME) "anime" else "manga"
        return apiService.search(typePath, includeNsfw, query)
    }

    override suspend fun getSeasonalAnime(partOfYear: PartOfYear, year: Int, includeNsfw: Boolean): Response<SeasonResponse> {
