...
class ListRecordConverter(private val context: Context) {
    ...
    fun transform(titleType: TitleType, record: ListRecord): RecordListViewEntity {
    ...
    fun transform(titleType: TitleType, records: List<ListRecord>): List<RecordListViewEntity> {

        val viewModels = ArrayList<RecordListViewEntity>()
        for (record in records) {
            viewModels.add(transform(titleType, record))
        }
        return viewModels
    }
}
