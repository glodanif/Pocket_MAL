...
import com.g.pocketmal.getTrimmedText
...
import com.g.pocketmal.ui.legacy.route.EditDetailsRoute
import com.g.pocketmal.ui.legacy.view.EditDetailsView
...
class EditDetailsActivity : SkeletonToolbarActivity(),
    EditDetailsView,
    EditDetailsRoute {

    private val recordId: Int by argument(EXTRA_RECORD_ID, 0)
    private val titleType: TitleType by transformedArgument<Int, TitleType>(EXTRA_TITLE_TYPE,
        TitleType.ANIME
    ) {
        TitleType.from(it)
    }

    private val presenter: EditDetailsPresenter by inject {
        parametersOf(
            recordId,
            titleType,
            this
    ...
    override fun openDatePicker(preset: Long, type: DateType) {
        com.g.pocketmal.ui.legacy.dialog.ClearButtonDatePickerDialog.show(this, preset) { _, year, month, dayOfMonth ->
