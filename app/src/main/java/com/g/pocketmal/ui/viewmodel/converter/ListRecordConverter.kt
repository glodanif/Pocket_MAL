...
class ListRecordConverter(private val context: Context) {
    ...
    fun transform(titleType: TitleType, record: DbListRecord, useEnglishTitle: Boolean): RecordListViewModel {
        ...
        return RecordListViewModel(
                ...
                episodesLabel = String.format("%s / %s", record.myEpisodes, if (record.seriesEpisodes != 0) record.seriesEpisodes.toString() else "—"),
                subEpisodesLabel = String.format("%s / %s", record.mySubEpisodes, if (record.seriesSubEpisodes != 0) record.seriesSubEpisodes.toString() else "—"),
