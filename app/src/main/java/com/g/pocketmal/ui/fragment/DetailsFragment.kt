...
class DetailsFragment : Fragment() {
    ...
    fun bindDetails(details: TitleDetailsViewModel) {
        ...
        val views = ArrayList<GenreView>()
        for (genre in details.genres) {
            val genreView = GenreView(requireActivity(), genre.id, genre.name)
    ...
    private fun setupStatsUi(item: TitleDetailsViewModel) {
    ...
    private fun getRelatedTitleView(relatedTitleItem: RelatedTitle, clickListener: (View) -> Unit): TextView {
        ...
        val textView = RelatedTitleView(requireActivity(), relatedTitleItem)
